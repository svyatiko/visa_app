...
def get_from_db(country):
    ...
    cur.execute(
        "SELECT vac_id FROM visa_application_centre WHERE country_id={}".format(
            county_id[0]
        )
    )
    ...
    for id in visa_application_centre_id:
        cur.execute(
            """SELECT DISTINCT c.name as CATEGORY, a.date, sc.name as SUBCAT 
                        FROM category c INNER JOIN sub_category sc ON c.id=sc.category_id
                        INNER JOIN appointment a ON a.subcat_id=sc.id WHERE a.vac_id={}""".format(
                id[0]
            )
        )
        ...
        print(res_str)
        cur.execute(
            "SELECT address FROM visa_application_centre WHERE id={}".format(id[0])
        )
