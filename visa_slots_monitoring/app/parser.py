...
def write_to_es(vac, category, sub_category, appointment, county):
    ...
    res = es.search(
        index="appointment", query=q
    )  # ['hits']['total']['value']#['hits'][0]['_id']
...
def vfs_login(driver):
    login = driver.find_elements(
        By.CLASS_NAME, "mat-input-element"
    )  # Найхожу поля ввода данных
    login[0].send_keys(EMAIL)  # EMAIL
    login[1].send_keys(PASSWORD)  # PASSWORD
    driver.find_elements(By.CLASS_NAME, "mat-focus-indicator")[
        1
    ].click()  # Клик по кнопке "Войти"
...
def get_visa_date(country, home="blr"):
    ...
    time.sleep(wait_sec)
    driver.execute_script(
        "arguments[0].click();", butt_vis[0]
    )  # Клик по полю выбора визовых центров
    ...
    for visa_cent_iter in range(len(visa_cent_list)):
        driver.execute_script(
            "arguments[0].click();", visa_cent_list[visa_cent_iter]
        )  # Клик по визовому центру

        time.sleep(wait_sec)
        driver.execute_script(
            "arguments[0].click();", butt_vis[1]
        )  # Клик по полю выбора категории записи
        ...
        for visa_cat_iter in range(len(visa_cat_list)):
            ...
            driver.execute_script(
                "arguments[0].click();", visa_cat_list[visa_cat_iter]
            )  # Клик по категории записи

            time.sleep(wait_sec)
            driver.execute_script(
                "arguments[0].click();", butt_vis[2]
            )  # Клик по полю выбора подкатегории записи

            visa_subcategory_list = driver.find_elements(
            ...
            for visa_subcategory_iter in range(len(visa_subcategory_list)):
                ...
                driver.execute_script(
                    "arguments[0].click();",
                    visa_subcategory_list[visa_subcategory_iter],
                )  # Клик по подкатегории записи

                try:
                    result = driver.find_element(
                        By.CLASS_NAME, "border-info"
                    )  # Нужное нам поле с данными о наличии записи
                except:
                    visa_cat_iter -= 1
                    time.sleep(5)
                    break
...
if __name__ == "__main__":
    ...
    schedule.every().day.at("15:00").do(
        perform_every_day
    )  # получение данных кажды день в 8:00
    while True:
